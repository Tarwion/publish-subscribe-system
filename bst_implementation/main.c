...
int main(int argc, char **argv){
	...
	T=malloc(hashTableSize*sizeof(subinfo*));
	...
	while (fgets(buff, BUFFER_SIZE, fin))
	{
		...
		case 'I':
		{
			...
			if(!flag){
				if (Insert_Info(groups,T,itm,iId,gids_arr,num_of_gids)==0)
				{
					DPRINT("%c <%d> <%d> DONE\n",buff[0], itm,iId);
				}
			}else
		...
		case 'S':
		{
			...
			if(!flag){
				if (Subscriber_Registration(T,groups,sTM,sId,gids_arr,num_of_gids)==0){
					DPRINT("%c <%d> <%d> DONE\n", buff[0],sTM,sId);
				}
            }else{
                fprintf(stderr, "%s failed\n", buff);
		...
		case 'D':
		{
			int sId;
			sscanf(buff, "%c %d", &event, &sId);
            if (Delete_Subscriber(groups,T,sId)==0)
			{
				printf("SUBSCRIBERLIST= %d \n",sId);
				for(int i=0;i<MG;i++){
					sub*temp=groups[i].gsub;
					printf("GROUPID= %d ,SUBLIST= ",i);
					while(temp!=NULL){
						printf("%d ",temp->sId);
						temp=temp->snext;
					}
