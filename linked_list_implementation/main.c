...
int main(int argc, char **argv)
{
	...
	while (fgets(buff, BUFFER_SIZE, fin))
	{
		...
		case 'I':
		{
            ...
            if(!flag){
                if(Insert_Info(groups,S,itm,iId,gids_arr,num_of_gids)==0){
				    DPRINT("%c <%d> <%d> DONE\n",buff[0], itm,iId);
			}
            }else{
		...
		case 'S':
		{
            ...
            if(!flag){
                if (Subscriber_Registration(&S,groups,sTM,sId,gids_arr,num_of_gids)==0){
                    DPRINT("%c <%d> <%d> DONE\n", buff[0],sTM,sId);
                }
            }else{
                fprintf(stderr, "%s failed\n", buff);
		...
		case 'D':
		{
			int sId;
			sscanf(buff, "%c %d", &event, &sId);
            if (Delete_Subscriber(&S,groups,sId)==0)
			{
				DPRINT("%c <%d> DONE\n", event, sId);
			}
			else
